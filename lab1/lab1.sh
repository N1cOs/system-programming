...
while :; do
		...
		case $OPTION in
				...
				"2")
						...
						get_file_name
						cd $FILE 2>> $ERR_PATH || echo "Error occured"
						;;
				"3")
				...
				"4")
						...
						get_file_name
						print_hl
						cat $FILE 2>> "$ERR_PATH" || echo "Error occurred"
						print_hl
						;;
				"5")
						...
						get_file_name
						cp $SOURCE_FILE $FILE 2>> "$ERR_PATH" || echo "Error occurred"
						;;
				"6")
						echo "Enter directory name"
						get_file_name
						mkdir "${FILE}" 2>> $ERR_PATH || echo "Error occurred"
						;;
				"7")
						DIR_INFO=$(ls -ld)
						...
						print_hl
						echo "Users who can read current directory:"
						print_hl
						echo "${can_read[@]}" | tr ' ' '\n' | sort -u | awk '{cmd = "id -u " $0; cmd | getline r; if (r >= 1000) print $0}'
						print_hl
						echo "Users who can write in the current directory:"
						print_hl
						echo "${can_write[@]}" | tr ' ' '\n' | sort -u | awk '{cmd = "id -u " $0; cmd | getline r; if (r >= 1000) print $0}'
						print_hl
